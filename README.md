...
- `exec` and `process` now also block the same internal `http(s)` targets when they appear inside obvious inline runtime fetch payloads such as `python -c` or `node -e`, so generic interpreters cannot trivially sidestep `web_fetch` policy.
