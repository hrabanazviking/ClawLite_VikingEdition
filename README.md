...
- `exec` and `process` now also block the same internal `http(s)` targets when they appear inside obvious inline runtime fetch payloads such as `python -c` or `node -e`, including common transparent launch wrappers like `/usr/bin/env`, `env -i`, `env -S`, `nohup`, `nice`, `timeout`, or `stdbuf`, so generic interpreters cannot trivially sidestep `web_fetch` policy.
