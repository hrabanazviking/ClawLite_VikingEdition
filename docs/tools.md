...
For `exec`, ClawLite now also derives approval-friendly specifiers from shell meta syntax, explicit shell wrappers (`sh -lc`, `bash -lc`, `cmd /c`), env override keys, and explicit cwd overrides. That lets operators write tighter rules such as `exec:shell` or `exec:env-key:git-ssh-command` instead of approving every `exec` call. The runtime also rejects dangerous env override pivots like `PATH`, `NODE_OPTIONS`, `DYLD_*`, `LD_*`, `GIT_CONFIG_*`, and `GIT_SSH_COMMAND`, and under `restrict_to_workspace` it now recursively guards nested shell commands instead of treating the wrapper as a safe boundary. It also blocks obvious `curl` / `wget` / PowerShell fetches to local, private, metadata, and other internal-only `http(s)` targets, plus clear inline runtime fetch payloads such as `python -c` or `node -e` that call network clients against those same destinations. That inline-runtime and fetch guard now resolves common transparent launch wrappers like `/usr/bin/env`, `env -i`, `env -S`, `command --`, `nohup`, `nice`, `timeout`, and `stdbuf`, so `exec`/`process` cannot sidestep the stricter network policy already enforced by `web_fetch`.
